...
from .network_blocks import BaseConv, DWConv, RFB2, PyramidPooling, FFM
...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        if self.segcls > 0:
            self.stems_seg = nn.ModuleList()
            self.seg4 = nn.Sequential(
                Conv(in_channels=int(256 * width), out_channels=int(128 * width), ksize=1, stride=1), )
            self.seg8 = nn.Sequential(
                Conv(in_channels=int(256 * width), out_channels=int(128 * width), ksize=1, stride=1),
                nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True), )
            self.seg16 = nn.Sequential(
                Conv(in_channels=int(256 * width), out_channels=int(128 * width), ksize=1, stride=1),
                nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True), )
            # self.seg32 = nn.Sequential(
            #     Conv(in_channels=int(256 * width), out_channels=int(128 * width), ksize=1, stride=1),
            #     nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True),)
            self.seg_preds = nn.Sequential(
                RFB2(int(128 * width) * 3, int(128 * width), d=[2, 3], map_reduce=6),
                # 3*128//6=64　RFB2和RFB无关，仅仅是历史遗留命名(训完与训练模型效果不错就没有改名重训了)
                PyramidPooling(int(128 * width), k=[1, 2, 3, 6]),  # 按原文1,2,3,6，PSP加全局更好，但是ASPP加了全局后出现边界破碎
                FFM(int(128 * width) * 2, int(128 * width), k=3, is_cat=False),
                Conv(in_channels=int(128 * width), out_channels=self.segcls, ksize=1, stride=1),
                nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
            )
        ...
        for i in range(len(in_channels)):
            ...
            if self.keypoints > 0:
                self.lmk_preds.append(
                    nn.Conv2d(
                        in_channels=int(256 * width),
                        out_channels=self.n_anchors * self.keypoints * 2,
                        kernel_size=1,
                        stride=1,
                        padding=0,
                    )
        ...
        self.seg_loss = CrossEntropyLoss2d(ignore_index=-1)
        self.seg_loss2 = DiceLoss()
    ...
    def forward(self, xin, labels=None, imgs=None, seg_labels=None):
        ...
        y_shifts = []
        expanded_strides = []
        seg_feature, seg_output = [], None
        if self.segcls > 0:
            for k, x in enumerate(xin):
                x0 = self.stems[k](x)
                if k == 0:
                    seg_feature.append(self.seg4(x0))
                elif k == 1:
                    seg_feature.append(self.seg8(x0))
                elif k == 2:
                    seg_feature.append(self.seg16(x0))
                # else:
                #     seg_feature.append(self.seg32(x0))
            xin = xin[1:]
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                expanded_strides.append(
                    torch.full((1, grid.shape[1]), stride_this_level, dtype=xin[0].dtype, device=xin[0].device)
                )
                if self.use_l1:
        ...
        if self.segcls > 0:
            seg_output = self.seg_preds(torch.cat(seg_feature, 1))
        ...
        if self.training:
            ...
            if self.decode_in_inference:
                return self.decode_outputs(outputs, seg_output=seg_output, dtype=xin[0].type())
            else:
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        n_ch = 5 + self.num_classes + self.keypoints * 2  # 5:x,y,w,h,bs
    ...
    def decode_outputs(self, outputs, seg_output, dtype):
        grids = []
        strides = []
        ...
        # if self.segcls > 0:
        #     seg_output = F.interpolate(seg_output, scale_factor=4, mode='bilinear')
        return outputs, seg_output
    ...
    def get_losses(
        ...
        num_fg = 0.0
        num_gts = 0.0
        _, _, self.im_h, self.im_w = imgs.shape
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                if self.segcls > 0:
                    # seg_target = outputs.new_zeros((1, self.im_h, self.im_w))
        ...
        if self.segcls > 0:
            ...
            loss_seg1 = self.seg_loss(seg_output, seg_targets.long())
            b, h, w = seg_targets.shape
            seg_targets_ls = []
            for b_ in range(b):
            ...
            seg_labels = torch.stack(seg_targets_ls, dim=0).cuda()
            loss_seg2 = self.seg_loss2(seg_output, seg_labels)
